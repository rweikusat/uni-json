...
different output formats for objects (holding key-value pairs) and arrays. UJ_FMT_FAST, the
...
UJ_FMT_DET uses I<deterministic> serialization for objects. The keys are sorted using
locale-blind codepoint comparions before serialization and key-value pairs will thus
always be serialized in the same order regardless of the order they were returned in
by the object traveral callback. The UJ_FMT_PRETTY format adds redundant whitespace

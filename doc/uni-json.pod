...
a JDON value at all (empty or only whitespace) or because text representing
...
The JSON text passed to C<uni_json_parse> didn't contain a single JSON value
...
because of an object lacking a B<:> separator between a key and the associated
value.

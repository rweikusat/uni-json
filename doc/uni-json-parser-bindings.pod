...
Called by the parser when it needs to create a string. A string must be capable of
holding an valid UTF-8 string, possibly including bytes with value 0 or other
control characters.
...
Called by the parser to create a B<null> value. Must return a pointer to one.

=item * C(void free_null(void *null)>

Called by the parser to free a B<null> value.

=item * C<void *make_bool(int true_false)>

Called by the parser to create a boolean. The value of the B<true_false> parameter will
...
Called by the parser to free a boolean value.

=item * C<void *make_number(uint8_t *data, size_t len, unsigned flags)>

Called by the parser to create a number. The sequence of bytes starting at

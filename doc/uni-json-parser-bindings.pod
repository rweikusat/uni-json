...
to use it in a particular runtime environment.
...
function B<croak>, can thus safely be called from B<on_error>.

=back

=head3 Object Creation/ Management

All functions supposed to create some kind of data object must either return
a pointer to suitable object or NULL to indicate that it couldn't be created.
...
B<make_object>. Should free all memory used for the object itself and whatever
...
to a string created by B<make_string>, B<value> will be pointer to a value created
by invoking one of the other B<make_...> functions and B<obj> a pointer to the object
...
created via B<make_array>. Should free all memory used for the array
...
be a pointer to another value created by invoking one of the B<make_...> functions.
...
To accomodate languages/ environments where strings are read-only, eg, Python, creating
a string is a 2-stage procedure. Upon encountering a JSON string, the parser will first
create a I<work string> object. It will then incrementally add UTF-8 data to it until
all of the input string has been processed. A call to the string finalization routine
is then supposed to create an actual string object from the collected data.

This facility will only be used for JSON strings containing escape sequences the
parser needs to convert into the corresponding UTF-8 data. Input strings without
them will be construced with a single call to C<finalize_string>.

=over

=item * C<void *make_work_string(void)>

Create a I<work string> object.

=item * C<void free_work_string(void *str)>

Called when the parser wants to disposed of a I<work string> object.

=item * C<int add_2_string(uint8_t *data, size_t len, void *str>)

Add the sequence of bytes starting at B<data> of length B<len> to the I<work string> pointed to
...
=item * C<void *(*finalize_string)(void *str, uint8_t *data, size_t len)>

Called to combine the content of the I<work string> object pointed to
by B<str> with a final chunk of UTF-8 data of length B<len> pointed to
by B<data> into an actual string object. The B<str> argument will NULL if
a I<work string> object wasn't need to handle a particular input string.

Must dispose of the object pointed to by B<str> if creation of the actual
string object succeeded.

=item * C<void (*free_string)(void *str)>

Called when the parser wants to dispose of a string object.
...
Called by the parser to create a I<null> value.

=item * C(void free_null(void *null)>

Called by the parser to free a I<null> value.

...
By the time the his handler is called, all memory implicitly allocated by calling the other handlers will already
have been freed, hence, it's save to call a function which causes a non-local control transfer, eg, the Perl API
function C<croak>, from it.

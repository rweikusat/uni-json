...
The function must set B<len> to the size of the array and B<p> to the
address of some kind of state information structure the language binding
code itself needs to enable the serializer to access the elements of
the array.

=item * C<void (*free_array_info)(void *p)>

If provided, the serializer will call this function after it's done
with handling this array and pass the B<p> pointer returned in the
...
uj_ary_info> for this array. The returned pointer must remain valid
until the next call to C<array_at> using the same B<p> or the final
call to C<free_array_info> for it.
...
of the string C<str> with a length of C<< data->len >> bytes. The serializer will escape
...
Objects of type C<UJ_T_NULL> will be serialized as I<null>.
